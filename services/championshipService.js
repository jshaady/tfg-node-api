...
const TeamMapper = require('../mapper/TeamMapper');
const teamMapper = new TeamMapper();
const Team = require('../models/Team');
const ClasificationTeam = require('../models/ClasificationTeam');
...
class ChampionshipService {
    ...
    */
    ...
    async createChampionship(req) {
        const championship = new Championship(null, req.body.championshipName, req.username, req.body.numMax, req.body.startInscription,
            req.body.endInscription, req.body.startChampionship, req.body.location, req.body.sport, req.body.type, null, null, req.body.description);
        try {
        ...
        return { message: championship.getType() + ' created successfully', 'id': id };
    }
    ...
    async getChampionship(req) {
        const championshipFromMapper = await championshipMapper.getChampionship(req.query.idChampionship, req.query.type);
        if (championshipFromMapper.championship.length < 1) {
            throw new NotFoundException('Incorrect championship name');
        } else {

            const championship = new Championship(championshipFromMapper.championship[0].id, championshipFromMapper.championship[0].championshipname,
                championshipFromMapper.championship[0].creatoruser, championshipFromMapper.championship[0].nummaxteams,
                dateformat(championshipFromMapper.championship[0].startinscription, "dd-mm-yyyy"), dateformat(championshipFromMapper.championship[0].endinscription, "dd-mm-yyyy"),
                dateformat(championshipFromMapper.championship[0].startchampionship, "dd-mm-yyyy"), championshipFromMapper.championship[0].location,
                championshipFromMapper.championship[0].sport, championshipFromMapper.championship[0].type, championshipFromMapper.championship[0].state,
                championshipFromMapper.championship[0].phase, championshipFromMapper.championship[0].description);
     ...
     */
    async join(req) {
        const joinData = await championshipMapper.checkJoin(req.body.idChampionship, req.body.teamname, req.username);
        if (joinData.championship.length < 1) {
            ...
            throw new ConflictException('Some of the team players already participate in the tournament');
     ...
     */
    async left(req) {
        const teamUsers = await championshipMapper.championshipTeamUsers(req.query.idChampionship, req.username);
    ...
    async participate(req) {
        ...
        if (teamJoined.length < 1) { return 'not_team'; }
        else {
            let teams = [];
            teamJoined.forEach(team => {
                teams.push(new Team(team.teamname, team.userleader, null, null, null, null, null, null,
                    null, null, null, null, null));
            });

            let participatePromise = [];
            teams.forEach(team => {
                participatePromise.push(championshipMapper.participate(req.query.idChampionship, team));
            });

            return Promise.all(participatePromise).then(values => {
                let joined = false;
                values.forEach(value => {
                    if (value === 2) { joined = true; }
                });
                if (joined) { return 'true'; }
                else if (!joined) { return 'false'; }
            });
        }
    }
    ...
    async getMatches(req) {
        ...
        const matchesFromMapper = await championshipMapper.getMaches(req.query.idChampionship, page, parseInt(req.query.pageSize), req.query.teamname);
        if (matchesFromMapper.totalMatches === 0) {
            ...
            matchesFromMapper.matches.forEach(match => {
                ...
                matches.push(new Match(req.query.idChampionship, match.phase, match.team1, match.team2,
                    match.matchresult1, match.matchresult2, matchDate, match.position));
                matchDate = null;
            });
            ...
            const pager = paginate(matchesFromMapper.totalMatches, page, parseInt(req.query.pageSize));
    ...
    async getBracketsMatches(req) {
        const matchesFromMapper = await championshipMapper.getBracketsMatches(req.query.idChampionship);
        if (matchesFromMapper.length === 0) {
            ...
            matchesFromMapper.forEach(matchFromMapper => {
                ...
                matches.push(new Match(req.query.idChampionship, matchFromMapper.phase, matchFromMapper.team1, matchFromMapper.team2,
                    matchFromMapper.matchresult1, matchFromMapper.matchresult2, matchDate, matchFromMapper.position));
                matchDate = null;
            });
    ...
    async generateMatches(req) {
        const championshipData = await championshipMapper.generateMatches(req.body.idChampionship);
        if (new Date() < championshipData.championship[0].endinscription) {
            ...
            if (championshipData.championship[0].type === 'League') {
                if (championshipData.teams.length > 1) {
                    ...
                    await championshipMapper.updateChampionship(updateChampionship, req.body.idChampionship);
                ...
                if (championshipData.teams.length > 1) {
                    ...
                    else if (4 < championshipData.teams.length && championshipData.teams.length <=  8) {
                        for (let i = 0; i < 4; i++) {
                            teamArray1.push(championshipData.teams[i]);
                        }
                        for (let i = 4; i < 8; i++) {
                    ...
                    else if (8 < championshipData.teams.length && championshipData.teams.length <= 16) {
                        for (let i = 0; i < 8; i++) {
                            teamArray1.push(championshipData.teams[i]);
                        }
                        for (let i = 8; i < 16; i++) {
                    ...
                    else if (16 < championshipData.teams.length && championshipData.teams.length <= 32) {
                        for (let i = 0; i < 16; i++) {
                            teamArray1.push(championshipData.teams[i]);
                        }
                        for (let i = 16; i < 32; i++) {
                ...
                throw new ConflictException('Incorrect championship type (not League or Tournament)');
            }
        }
    }
    ...
    async clasification(req) {
        const championshipFromMapper = await championshipMapper.getChampionship(req.query.idChampionship, 'League');
        if (championshipFromMapper.championship.length < 1) {
            throw new NotFoundException('Incorrect championship name');
        } else {
            const championshipData = await championshipMapper.clasification(req.query.idChampionship);
     ...
     */
    async setResult(req) {
        const match = await championshipMapper.getMatch(req.body.idChampionship, req.body.team1, req.body.team2);
        if (match.length < 1) {
            throw new NotFoundException('Match not found');
        } else {
            if (match[0].phase !== null && parseInt(req.body.matchResult1) === parseInt(req.body.matchResult2)) {
                throw new ConflictException('Cannot set a draw in tournament match');
            }
            if (parseInt(req.body.matchResult1) < 0 || parseInt(req.body.matchResult2) < 0){
            ...
            await championshipMapper.setResult(updateMatch, req.body.idChampionship, req.body.team1, req.body.team2);
            if (match[0].phase === 'Final') {
                const updateChampionship = {
                    state: 'Finished'
                }
                await championshipMapper.updateChampionship(updateChampionship, match[0].idchampionship);
            }
            if (match[0].phase === null) {
                const leagueMatchesNotFinished = await championshipMapper.leagueFinishedMatches(match[0].idchampionship);
     ...
     */
    async addDate(req) {
        const teamsLeaders = await championshipMapper.getTeamsLeader(req.body.team1, req.body.team2);
        if (teamsLeaders.team1FromMapper[0].userleader !== req.username) {
            throw new ConflictException('You not the team leader');
        } else {
            const matchDatesData = await championshipMapper.checkMatchDates(req.body.idChampionship, req.body.team1, req.body.team2,
    ...
    async deleteDate(req) {
        const teamsLeaders = await championshipMapper.getTeamsLeader(req.query.team1, req.query.team2);
        if (teamsLeaders.team1FromMapper[0].userleader !== req.username) {
            throw new ConflictException('You not the team leader');
        } else {
            const matchDatesData = await championshipMapper.checkMatchDates(req.query.idChampionship, req.query.team1, req.query.team2,
                dateformat(req.query.date, "yyyy-mm-dd HH:MM"));
            if (matchDatesData.matchDateCount === 0) {
                throw new NotFoundException('Date not exists');
            }
            else {
                await championshipMapper.deleteMatchDate(req.query.idChampionship, req.query.team1, req.query.team2,
                    dateformat(req.query.date, "yyyy-mm-dd HH:MM"));
    ...
    async acceptDate(req) {
        const teamsLeaders = await championshipMapper.getTeamsLeader(req.body.team1, req.body.team2);
        if (teamsLeaders.team1FromMapper.length < 1 || teamsLeaders.team2FromMapper.length < 1) {
            throw new NotFoundException('Incorrect team name');
        } else if (teamsLeaders.team2FromMapper[0].userleader !== req.username) {
            throw new ConflictException('You not the team leader');
        } else {
            const matchFromMapper = await championshipMapper.getMatch(req.body.idChampionship, req.body.team1, req.body.team2);
            if (matchFromMapper.length < 1) {
                ...
                if (matchDatesData.matchDateCount < 1) {
                    ...
                    await championshipMapper.updateMatch(updateMatch, req.body.idChampionship, req.body.team1, req.body.team2);
                    await championshipMapper.deleteMatchDates(req.body.idChampionship, req.body.team1, req.body.team2);
    ...
    async getMatchDates(req) {
        ...
        if (teamsLeaders.team1FromMapper.length < 1 || teamsLeaders.team2FromMapper.length < 1) {
            throw new NotFoundException('Incorrect team name');
        } else if (teamsLeaders.team1FromMapper[0].userleader !== req.username && teamsLeaders.team2FromMapper[0].userleader !== req.username) {
            throw new ConflictException('You not the team leader');
        } else {
            const matchDatesData = await championshipMapper.getMatchDates(req.query.idChampionship, req.query.team1, req.query.team2);
            const matchDates = [];
    ...
    async generateNextPhase(req) {
        const championshipData = await championshipMapper.getChampionship(req.body.idChampionship, 'Tournament');
        if (championshipData.championship.length < 1) {
            ...
            const matches = await championshipMapper.getMachesByPhase(championshipData.championship[0].id, championshipData.championship[0].phase);
            if (matches.length < 1) {
                ...
                await championshipMapper.updateChampionship(updateChampionship, championshipData.championship[0].id);
    ...
    async getAllChampionships(req) {
        const championshipsFromMapper = await championshipMapper.getAllChampionships(req.query.type, req.query.name, req.query.location, 
                req.query.sport, parseInt(req.query.page));
        if (championshipsFromMapper.championships.length < 1) {
            return [];
        } else {
            const championships = [];
            championshipsFromMapper.championships.forEach(championship => {
                championships.push(new Championship(championship.id, championship.championshipname, championship.creatoruser, championship.nummaxteams,
                    dateformat(championship.startinscription, "dd-mm-yy HH:MM"), dateformat(championship.endinscription, "dd-mm-yy HH:MM"),
                    dateformat(championship.startchampionship, "dd-mm-yy HH:MM"), championship.location, championship.sport, championship.type, championship.state,
                    championship.phase));
            });
            const pageChecked = parseInt(req.query.page) || 1;
            const pageSize = 20;
            const pager = paginate(championshipsFromMapper.totalChampionships, pageChecked, pageSize);
    ...
    async getUserNextMatches(req) {
        ...
        const matchesFromMapper = await championshipMapper.getUserNextMatches(req.username,
            dateformat(req.query.date, 'dd-mm-yyyy HH:mm:ss'), parseInt(req.query.pageIndex), parseInt(req.query.pageSize), req.query.type);
        if (matchesFromMapper.totalMatches === 0) {
            return [];
        } else {
            const matches = [];
            matchesFromMapper.championshipMatches.forEach(match => {
                matches.push(new Match(match.idchampionship, match.phase, match.team1, match.team2,
                    null, null, dateformat(match.matchdate, 'dd-mm-yyyy HH:mm'), null));
            });

            const page = parseInt(req.query.pageIndex) < 0 ? 1 : parseInt(req.query.pageIndex);
            const pager = paginate(matchesFromMapper.totalMatches, page, parseInt(req.query.pageSize));
