...
export default class ChatService {
  ...
  async getMessages(req) {
    const messages = await chatMapper.getMessages(
      req.username,
      req.query.userTarget
    );
    if (messages.length === 0) {
      ...
      messages.forEach((message) => {
        if (lastMessageUser === message.senduser) {
          messageGroup.setMessageInArray({
          ...
          messageGroup = new Message(
            message.senduser,
            message.targetuser,
            null,
            dateformat(message.senddate, "dd-mm-yyyy HH:MM")
          );
          messageGroup.setMessageInArray({
            messageContent: message.messagecontent,
          });
