...
class TeamsService {
    ...
    */
    ...
    async createTeam(req) {
        const team = new Team(req.body.teamname, req.username, req.body.description, req.body.date, req.body.location, null, null,
            req.body.isPrivate, null, req.body.maxNumberPlayers);
        const teamFromMapper = await teamMapper.getTeam(team.getTeamname());
        if (teamFromMapper.length > 0) {
            throw new ConflictException('Conflict data', {teamExists: 'This team name already exists'});
        } else {
            try {
    ...
    */
    async editTeam(req) {
        const team = new Team(req.body.teamname, null, req.body.description, null, req.body.location);
        const teamFromMapper = await teamMapper.getTeam(team.getTeamname());
        if (teamFromMapper.length === 0) {
            throw new ConflictException('Incorrect team name');
        } else if (teamFromMapper[0].userleader !== req.username) {
            throw new ConflictException('You not the team leader, cant edit the info');
        } else {
            try {
    ...
    */
    async getTeam(req) {
        const teamFromMapper = await teamMapper.getTeamWithUsers(req.query.teamname);
        if (teamFromMapper.length < 1) {
            ...
            const team = new Team(teamFromMapper[0].teamname, teamFromMapper[0].userleader, description, dateformat(teamFromMapper[0].createdate, "dd-mm-yyyy"), teamFromMapper[0].location,
                teamFromMapper[0].imagebase64, teamFromMapper[0].imagetype, teamFromMapper[0].isprivate, null, teamFromMapper[0].nummaxplayers);
    ...
    async getAllMyTeams(req) {
        ...
        if (teamsFromMapper.length < 1) {
            ...
            teamsFromMapper.forEach(team => {
                teams.push(new Team(team.teamname, team.userleader, null, dateformat(team.createdate, "dd-mm-yyyy"), team.location,
                    team.imagebase64, team.imagetype, team.isprivate, null, team.nummaxplayers));
            });
            return teams;
        }
    }
    ...
    */
    async getMyTeamsCreated(req) {
        ...
        if (teamsFromMapper.length < 1) { return []; }
        else {
            const teams = [];
            teamsFromMapper.forEach(team => {
                teams.push(new Team(team.teamname, team.userleader, null, null, null, null, dateformat(team.createdate, "dd-mm-yyyy"), team.location,
                    team.imagebase64, team.imagetype, team.isprivate, null, team.nummaxplayers));
            });
            return teams;
        }
    ...
    */
    async searchByUuid(req) {
        ...
        if (teamFromMapper.length < 1) {
            ...
            return new Team(teamFromMapper[0].teamname, null, null, teamFromMapper[0].createdate, teamFromMapper[0].location,
                teamFromMapper[0].teamAvatarBase64, teamFromMapper[0].imagetype, null, teamFromMapper[0].uuid, teamFromMapper[0].nummaxplayers);
        }
    }
    ...
    */
    async search(req) {
        const teamsFromMapper = await teamMapper.search(req.query.teamname, req.query.location, req.username);
        if (teamsFromMapper.length < 1) {
            return [];
        } else {
            const teams = [];
            teamsFromMapper.forEach(team => {
                teams.push(new Team(team.teamname, team.userleader, null, dateformat(team.createdate, "dd-mm-yyyy"), team.location,
                    team.imagebase64, team.avatarType, team.isprivate, null, team.nummaxplayers))
            });
            return teams;
        }
    }
    ...
    */
    async join(req) {
        const dataIsInTeam = await teamMapper.isInTeam(req.body.teamname, req.username);
        ...
        } else if (dataIsInTeam.team[0].isprivate === 1 && dataIsInTeam.team[0].uuid !== req.body.uuid) {
            throw new ConflictException('Incorrect team code');
        } else if (dataIsInTeam.numberOfPlayers[0].totalplayers + 1 > dataIsInTeam.team[0].nummaxplayers) {
    ...
    */
    async left(req) {
        const dataIsInTeam = await teamMapper.isInTeam(req.query.teamname, req.username);
        if (dataIsInTeam.team.length < 1) {
            throw new NotFoundException('Team not found');
    ...
    */
    async stats(req) {
        ...
        if (matchesPlayed.length < 1) {
            ...
            const matchesPlayedData = await teamMapper.getChampionshipPlayed(req.query.teamname);
            return {
                'matches': matches,
