...
class MeetService {
    ...
    */
    async createMeet(req) {
        const meet = new Meet(null, req.body.name, req.username, req.body.date, req.body.sport, req.body.description, req.body.location);
        try {
        ...
        if (meetData.meet.length === 0) { 
            ...
            if (meetData.users.length > 0) {
                meetData.users.forEach(user => {
                    usersInMeet.push(new User(user.username, user.name, user.surname, null, null,
                        null, null, null, user.imagebase64, user.imagetype, null));
                });
            }
            const meet = new Meet(meetData.meet[0].id, meetData.meet[0].name, meetData.meet[0].usercreator, meetData.meet[0].date,
                meetData.meet[0].sport, meetData.meet[0].description, meetData.meet[0].location);
    ...
    */
    async getMeets(req) {
        ...
        const meetsFromMapper = await meetMapper.getMeets(req.query.location, page, parseInt(req.query.pageSize), dateformat(req.query.date, 'yyyy-mm-dd HH:mm'));
        if (meetsFromMapper.totalMeets === 0) {
            ...
            meetsFromMapper.meets.forEach(meet => {
                ...
                let meetSave = new Meet(meet.id, meet.name, meet.usercreator, meetDate, meet.sport,
                    meet.description, meet.location);
                meetSave.setParticipants(meet.participants)
                meets.push(meetSave);
            });

            const pager = paginate(meetsFromMapper.totalMeets, parseInt(req.query.pageIndex), parseInt(req.query.pageSize));
    ...
    */
    async joinMeet(req) {
        const existsData = await meetMapper.existsInMeetUsers(req.username, req.body.idMeet);
    ...
    */
    async leftMeet(req) {
        const existsData = await meetMapper.existsInMeetUsers(req.username, req.query.idMeet);
        if (existsData.meet.length === 0) {
            return new NotFoundException('Meet not found');
        }
