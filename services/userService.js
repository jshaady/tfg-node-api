...
import UnauthorizedRequestException from "../models/UnauthorizedRequestException.js";
...
export default class UserService {
  ...
  async getUser(req) {
    ...
    if (data.user.length === 0) {
      ...
      const user = new User(
        ...
        data.user[0].rol,
        data.user[0].imagebase64,
        data.user[0].imagetype,
  ...
  async getProfile(req) {
    ...
    if (data.user.length === 0) {
      ...
      const user = new User(
        ...
        data.user[0].surname,
        data.user[0].email,
        dateFormat(data.user[0].birthdate, "yyyy-mm-dd"),
        data.user[0].country,
        data.user[0].location,
        data.user[0].rol,
        data.user[0].imagebase64,
  ...
  async signIn(req) {
    ...
    if (userSearched.length === 0) {
      ...
      throw new ConflictException("Not confirmed email", {
        email: userSearched[0].email,
      });
    } else {
      const validPassword = await bcrypt.compare(
        req.body.password,
        userSearched[0].password
      );
      if (!validPassword) {
        throw new ConflictException("Incorrect password");
      } else {
        const user = new User(
          userSearched[0].username,
          userSearched[0].name,
          userSearched[0].surname,
          userSearched[0].email,
          dateFormat(userSearched[0].birthdate, "yyyy-mm-dd"),
          userSearched[0].country,
          userSearched[0].location,
          userSearched[0].rol,
          userSearched[0].imagebase64,
          userSearched[0].imagetype,
          userSearched[0].description
        );
        const token = jwt.sign(
          { username: user.getUsername(), rol: user.getRol() },
          "secret",
          { expiresIn: "2h" }
        );
        user.setToken(token);
  ...
  async signUp(req) {
    if (req.body.password === null) {
      throw new ConflictException("Incorrect data", {
        error: "The password cannot be null",
      });
    } else if (req.body.password.length < 5) {
      throw new ConflictException("Incorrect data", {
        passwordMinLength: "The password cannot have less than 4 characters",
      });
    } else if (req.body.password.length > 25) {
      throw new ConflictException("Incorrect data", {
        passwordMaxLength: "The password cannot have more than 25 characters",
      });
    } else {
      const user = new User(
        req.body.username,
      ...
      if (userExists.length === 1) {
        ...
        const emailInfo = {
          ...
          html:
            'Please click this link to confirm your email: <a href="' +
            url +
            '">' +
            url +
            "</a>",
  ...
  async getUsersList(req) {
    if (req.rol != 1) {
      ...
      const data = await userMapper.getUsersList(
        page,
        parseInt(req.query.pageSize),
        req.query.nameOrEmail
      );
      let users = [];
   ...
   */
  async getStats(req) {
    const matchesPlayed = await userMapper.getMatchesPlayed(
    ...
    if (matchesPlayed.length < 1) {
      ...
      const statsData = await userMapper.getStats(
        req.query.username.trim(),
        req.query.sport
      );
      return {
  ...
  async events(req) {
    if (req.query.type === "meets") {
      const meetsFromMapper = await meetMapper.getUserNextMeets(
        req.username,
      ...
      meetsFromMapper.forEach((meet) => {
        nextMeets.push(
          new Meet(
            ...
            meet.name,
            meet.usercreator,
            dateFormat(meet.date, "dd-mm-yyyy HH:mm:ss"),
            meet.sport,
            meet.description,
            meet.location
          )
      ...
      const championshipFromMapperData =
        await championshipMapper.getUserNextChampionships(
          req.username,
          dateFormat(req.query.date, "yyyy-mm-dd HH:mm:ss"),
          req.query.page,
          req.query.type
        );
