...
export default class ChampionshipMapper {
  /**
  ...
  async getChampionship(id, type) {
    try {
      const [championhsip] = await connection.query(
        "SELECT * FROM championships WHERE id = ? AND type = ?",
        [id, type]
      );
      const [championshipTeams] = await connection.query(
        "SELECT * FROM championshipteams WHERE idchampionship = ?",
        [id]
      );
      return {
  ...
  async checkJoin(id, teamname, username) {
    try {
      const [championship] = await connection.query(
        "SELECT * FROM championships WHERE id = ?",
        [id]
      );
      const [isInChampionship] = await connection.query(
        "SELECT COUNT(*) AS count FROM teams t JOIN championshipteams c JOIN teamusers u WHERE t.teamname = c.teamname AND " +
          "t.teamname=u.teamname AND c.idchampionship = ? AND u.username IN (SELECT username FROM teamusers WHERE teamname = ?)",
        [id, teamname]
      );
      const [isLeader] = await connection.query(
        "SELECT * FROM teams WHERE teamname = ? AND userleader = ?",
        [teamname, username]
      );
      const [currentJoined] = await connection.query(
        "SELECT COUNT(*) AS count FROM championshipteams WHERE idchampionship = ?",
        [id]
      );
      return {
  ...
  async join(insertChampionshipTeam) {
    try {
      await connection.query("INSERT INTO championshipteams SET ?", [
        insertChampionshipTeam,
      ]);
      return;
    } catch (e) {
      throw new InternalErrorException("Internal error");
  ...
  async championshipTeamUsers(id, username) {
    const [teamUsers] = await connection.query(
      "SELECT * FROM teamusers tu JOIN championshipteams ct WHERE tu.teamname = ct.teamname " +
        "AND tu.userName = ? AND ct.idchampionship = ?",
      [username, id]
    );
    return teamUsers;
  }
  ...
  async left(id, teamname) {
    try {
      await connection.query(
        "DELETE FROM championshipteams WHERE teamname = ? AND idchampionship = ?",
        [teamname, id]
      );
      return;
    } catch (e) {
  ...
  async participate(idchampionship, team) {
    try {
      const [participate] = await connection.query(
        "SELECT * FROM championshipteams WHERE idchampionship = ? AND teamname = ?",
  ...
  async getMaches(id, pageIndex, pageSize, teamname) {
    try {
      ...
      const [count] = await connection.query(
        "SELECT COUNT(*) as count FROM matches WHERE idchampionship = ?" +
          (teamname !== "null" ? " AND (team1 = ? OR team2 = ?)" : ""),
        countParams
      );
      ...
      const matchesParams =
        teamname !== "null"
          ? [id, teamname, teamname, limit, offsetValue]
          : [id, limit, offsetValue];
      const [matches] = await connection.query(
        "SELECT * FROM matches WHERE idchampionship = ?" +
          (teamname !== "null" ? " AND (team1 = ? OR team2 = ?)" : "") +
          " ORDER BY position, matchdate DESC LIMIT ? OFFSET ?",
        matchesParams
      );
  ...
  async getBracketsMatches(id) {
    try {
      const [matches] = await connection.query(
        "SELECT * FROM matches WHERE idchampionship = ? ORDER BY phase, position, matchdate DESC",
        [id]
      );
      return matches;
    } catch (e) {
  ...
  async updateMatch(updateMatch, idChampionship, team1, team2) {
    try {
      await connection.query(
        "UPDATE matches SET ? WHERE idchampionship = ? AND team1 = ? AND team2 = ?",
  ...
  async deleteMatchDates(idChampionship, team1, team2) {
    try {
      await connection.query(
        "DELETE FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ?",
  ...
  async deleteMatchDate(idChampionship, team1, team2, date) {
    try {
      await connection.query(
        "DELETE FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ? AND date = ?",
  ...
  async getMatchTeams(match) {
    try {
      const [team1] = await connection.query(
        "SELECT * FROM teams WHERE teamname = ?",
        [match.getTeam1()]
      );
      const [team2] = await connection.query(
        "SELECT * FROM teams WHERE teamname = ?",
        [match.getTeam2()]
      );
  ...
  async createMatch(insertMatch) {
    try {
      connection.query("INSERT INTO matches SET ?", [insertMatch]);
      return;
    } catch (e) {
      throw new InternalErrorException("Internal error");
    }
  ...
  async updateChampionship(updateChampionship, id) {
    try {
      await connection.query("UPDATE championships SET ? WHERE id = ?", [
        updateChampionship,
        id,
      ]);
      return;
  ...
  async getMatch(idChampionship, team1, team2) {
    try {
      const [match] = await connection.query(
        "SELECT * FROM matches WHERE idchampionship = ? AND team1 = ? AND team2 = ?",
        [idChampionship, team1, team2]
      );
      return match;
    } catch (e) {
  ...
  async leagueFinishedMatches(idChampionship) {
    try {
      const [leagueMatchesNotFinished] = await connection.query(
        "SELECT COUNT(*) AS count FROM matches WHERE idchampionship = ? AND matchresult1 IS NULL AND matchresult2 IS NULL",
        [idChampionship]
      );
  ...
  async getMachesByPhase(id, phase) {
    try {
      const [matches] = await connection.query(
        "SELECT * FROM matches WHERE idchampionship = ? AND phase = ? ORDER BY position",
        [id, phase]
      );
      return matches;
    } catch (e) {
  ...
  async checkMatchDates(idChampionship, team1, team2, date) {
    try {
      const [matchDatesCount] = await connection.query(
        "SELECT COUNT(*) AS count FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ?",
        [idChampionship, team1, team2]
      );
      const [matchDateCount] = await connection.query(
        "SELECT COUNT(*) AS count FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ? AND date = ?",
        [idChampionship, team1, team2, date]
      );
      return (date = {
  ...
  async getMatchDates(idChampionship, team1, team2) {
    try {
      const [matchDates] = await connection.query(
        "SELECT * FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ?",
        [idChampionship, team1, team2]
      );
      return matchDates;
    } catch (e) {
  ...
  async getMatchDate(idChampionship, team1, team2, date) {
    try {
      const [matchDate] = await connection.query(
        "SELECT * FROM matchdates WHERE idchampionship = ? AND team1 = ? AND team2 = ? AND date = ?",
        [idChampionship, team1, team2, date]
      );
      return matchDate;
    } catch (e) {
  ...
  async addDate(matchDateInsert) {
    try {
      await connection.query("INSERT INTO matchdates SET ?", [matchDateInsert]);
      return;
    } catch (e) {
      throw new InternalErrorException("Internal error");
    }
  ...
  async getTeamsLeader(team1, team2) {
    try {
      const [team1FromMapper] = await connection.query(
        "SELECT userleader FROM teams WHERE teamname = ?",
        [team1]
      );
      const [team2FromMapper] = await connection.query(
        "SELECT userleader FROM teams WHERE teamname = ?",
        [team2]
      );
      return {
  ...
  async getAllChampionships(type, name, location, sport, page) {
    try {
      ...
      const queryParameters =
        name !== "null" && location !== "null" && sport !== "all"
          ? [type, "%" + name + "%", "%" + location + "%", sport]
          : name === "null" && location !== "null" && sport !== "all"
          ? [type, "%" + location + "%", sport]
          : name !== "null" && location === "null" && sport !== "all"
          ? [type, "%" + name + "%", sport]
      ...
      const countQuery =
        "SELECT COUNT(*) AS count FROM championships WHERE " +
      ...
      const championshipsQuery =
        "SELECT * FROM championships WHERE " +
        '(state = "In progress" OR state = "Inscription") AND type = ? ' +
        (name !== "null" ? "AND championshipname LIKE ? " : "") +
        (location !== "null" ? "AND location LIKE ? " : "") +
        (sport !== "all" ? "AND sport = ? " : "") +
  ...
  async getUserNextChampionships(username, date, page, type) {
    try {
      const [championshipData] = await connection.query(
        "SELECT DISTINCT c.id, tu.teamname, c.championshipname, c.id FROM teamusers tu JOIN championshipteams ct JOIN championships c JOIN matches m " +
          ...
          "m.matchdate > ? ORDER BY m.matchdate LIMIT ? OFFSET ?",
        [username, type, date, 5, page * 5]
      );
  ...
  async getUserNextMatches(username, date, pageIndex, pageSize, type) {
    try {
      const [count] = await connection.query(
        "SELECT COUNT(*) AS count FROM teamusers tu JOIN championshipteams ct JOIN championships c JOIN matches m " +
          "WHERE tu.username = ? AND tu.teamname = ct.teamname AND ct.idchampionship = c.id AND c.id = m.idchampionship AND " +
          "(tu.teamname = m.team1 || tu.teamname = m.team2) AND c.type = ? AND m.matchresult1 IS NULL AND m.matchdate IS NOT NULL AND " +
          "m.matchdate > ?",
        [username, type, date]
      ...
      const [championshipMatches] = await connection.query(
        "SELECT * FROM teamusers tu JOIN championshipteams ct JOIN championships c JOIN matches m " +
          "WHERE tu.username = ? AND tu.teamname = ct.teamname AND ct.idchampionship = c.id AND c.id = m.idchampionship AND " +
          "(tu.teamname = m.team1 || tu.teamname = m.team2) AND c.type = ? AND m.matchresult1 IS NULL AND m.matchdate IS NOT NULL AND " +
          "m.matchdate > ? ORDER BY m.matchdate LIMIT ? OFFSET ?",
        [username, type, date, limit, offsetValue]
      );
      return {
        totalMatches: count[0].count,
