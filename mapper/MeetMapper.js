...
class MeetMapper {
    ...
    */
    ...
    async createMeet(meet) {
        try {
            const [create] = await this.connection.query('INSERT INTO meets SET ?', [meet]);
            return create;
        } catch (e) {
    ...
    async getMeet(id) {
        try {
            const [meet] = await this.connection.query('SELECT * FROM meets WHERE id = ?', [id]);
            const [users] = await this.connection.query('SELECT * FROM meetusers m JOIN users u WHERE m.username = u.username && idmeet = ?', [id]);
    ...
    */
    async getMeets(location, pageIndex, pageSize, date) {
        try {
            const [count] = await this.connection.query('SELECT COUNT(*) AS count FROM meets WHERE location = ? && ? < date', [location, date]);
            ...
            const [meets] = await this.connection.query('SELECT id, name, usercreator, sport, description, location, date, ' +
             '(SELECT COUNT(*) FROM meetusers WHERE idmeet = id) as participants FROM meets WHERE location = ? && ? < date ORDER BY date LIMIT ? OFFSET ?', [location, date, limit, offsetValue]);
    ...
    */
    async joinMeet(meetJoin) {
        try {
            await this.connection.query('INSERT INTO meetusers SET ?', [meetJoin]);
            return;
        } catch (e) {
            throw new InternalErrorException('Internal error');
        }
    ...
    */
    async leftMeet(username, idMeet) {
        try {
            await this.connection.query('DELETE FROM meetusers WHERE username = ? && idmeet = ?', [username, idMeet]);
            return;
        } catch (e) {
            throw new InternalErrorException('Internal error');
        }
    ...
    */
    async existsInMeetUsers(username, idMeet) {
        try {
            const [meet] = await this.connection.query('SELECT * FROM meets WHERE id = ?', [idMeet]);
            const [userInMeet] = await this.connection.query('SELECT * FROM meetusers WHERE username = ? && idmeet = ?', [username, idMeet]);
    ...
    */
    async usersInMeet(meet) {
        try {
            const [usersInMeet] = await this.connection.query('SELECT COUNT(*) AS number FROM meetusers WHERE idmeet = ?', [meet.getId()]);
    ...
    */
    async getUserNextMeets(username, date, page) {
        try {
            const [meets] = await this.connection.query('SELECT * FROM meets m LEFT JOIN meetusers mu ON m.id = mu.idmeet WHERE mu.username = ? ' +
                'AND ? < m.date ORDER BY m.date LIMIT ? OFFSET ?', [username, date, 5, 5 * page]);
            return meets;
        }
        catch (e) {
            throw new InternalErrorException('Internal error');
