...
export const socketConnection = (socket) => {
  ...
  socket.on("sendMessage", (data) => {
    chatService.sendMessage(socket.request.user, data).then(() => {
      usersConnected.get(socket.request.user).socket.emit("updateMessages", {
        userSend: socket.request.user,
        userTarget: data.usernameTarget,
      });
      if (usersConnected.get(data.usernameTarget)) {
        usersConnected.get(data.usernameTarget).socket.emit("updateMessages", {
          userSend: socket.request.user,
          userTarget: data.usernameTarget,
        });
      }
